  ...
  "papers": [
    ...
    },
    {
      ...
      "evidence": [
        ...
        {
          "claim": "SGLang Serving 实验在单张 H20 上测试 Qwen3-8B 的不同并发吞吐量。",
          "location": "Appendix A.4 and Table 3, Pages 11–12",
          "type": "system"
        }
      ],
      "notes": [
        "Qwen3-8B 的平均接受长度相对 DFlash 提升约 14.7%，但最终速度只提升约 8.1%，说明更深 Drafter 的额外计算消耗了部分接受收益。",
        "结构消融中，完整方法平均 Speedup/Acceptance Length 为 3.37×/4.52；去掉 Softmax、异构 KV Projection 或渐进 Loss 后分别约为 3.31×/4.47、3.35×/4.48 和 3.34×/4.47。单个组件有效，但没有任何一项单独解释全部提升。",
        "训练数据扩展带来的增益非常显著：Qwen3-8B 从 800K 扩展到 2.4M 后，平均速度由 4.97× 提升到 5.46×，接受长度由 6.59 提升到 7.33。评价 DFlare 时应把架构扩展和数据扩展分开看。",
        "论文正文称 DFlare 在 SGLang 的所有并发设置下吞吐最高，但 Table 3 中 HumanEval、Concurrency=1 时 DFlare 为 554.5 tokens/s，略低于 DFlash 的 561.1 tokens/s。",
        "DFlare 改善的是 Drafter 容量和 Target 条件注入，与 DDTree 的候选树组织较为正交；它本身没有处理动态验证预算和高负载调度。"
      ],
      "provenance": {
