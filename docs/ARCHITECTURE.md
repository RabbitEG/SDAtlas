...
| 原始单位 | Excel D 列 | `workbookInstitutions` | 追溯源表、数据校验 |
...
单位排序默认采用“共同一作 / 一作单位 → 通讯或资深作者单位 → 其他作者单位”。论文通常没有机构贡献比例声明，所以 `order` 表示导航用的近似排序，不表示百分比。`ui.js` 从明细生成带编号的单位 Chip，复用全站 Tooltip；原 Excel D 列同时展示，便于发现源表历史信息与校正值的差异。
