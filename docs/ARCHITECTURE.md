...
- `relations`：分别记录 `extends`、`comparesAgainst`、`related` 与 `compatibleWith`；不能由引用边自动推断。
...
- `citations` 只表示论文是否引用；`relations` 只表示方法谱系或明确的可组合关系。
...
5. 只加入已核实且已收录的 `citations` 目标；不得据此自动生成 `relations`。
