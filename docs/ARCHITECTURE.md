...
- `ui.js` 负责通用查找、路由、论文卡片、Tooltip 和 MathML；`citation-graph.js` 虽保留历史文件名，但负责四类论文关系的传递约简、拓扑分层、直角避让连线、关系提示以及缩放和平移，各页面脚本只管理自身状态与布局。
...
- `citations` 只表示论文是否引用；`relations` 表示关系图使用的论文关系。当前二者可能因一次性初始化而相同，但语义和后续维护仍然独立。
...
源文件若手写这些字段，构建会失败。`citedBy` 和引用计数继续只由 `citations` 派生；关系图直接读取源文件中的 `relations`。
...
同一来源和目标可以同时具有多种 relation，显示时合并为一条边并列出全部类型。尚未人工细分关系的旧条目以 `citations` 一次性初始化 `related`，以保持迁移前的图结构；已有人工关系分类的条目不参与这次复制，也不会建立永久同步规则。

### 显示层约简与布局

`citation-graph.js` 每次接收当前页面的论文集合后，从四类 `relations` 构造集合内部的原始边。双方重复记录的纯 `related` 关系会先在显示层合并为一条稳定方向的边；随后逐类型检查是否存在不使用当前边的替代路径，只有相同 relation 类型存在替代路径时才省略直接边。源数据及生成数据始终保持完整。若其他有向关系意外形成环，环内边仍会保留，避免对非 DAG 图做不唯一的强制约简。

约简后的图以强连通分量压缩图计算拓扑层级：关系目标位于左侧，关系来源位于右侧。层内使用多轮重心排序降低交叉；所有边均为直角折线，相邻层连线只经过列间空隙，跨层连线走节点区域上方或下方的独立通道，尽量避免穿过论文块。箭头使用较小 Marker；透明加宽命中线保证低缩放下仍可悬停或键盘聚焦。放大到详细比例会显示 relation 字段名，悬停或聚焦则显示来源、目标、字段名和中文含义。
...
- `paper.html?id=<paper-id>`：单篇论文的完整作者、方法概述、贡献、单位、链接、引用计数与论文关系图。
...
5. 结果、限制、关系、复现和证据字段的枚举、URL 与站内 ID；
