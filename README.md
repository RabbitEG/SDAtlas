...
SDAtlas 是一个零依赖、可直接静态部署的投机解码论文图谱。站点以统一的 A–E“子问题”组织论文，同时提供完整论文目录、单篇论文页、站内引用脉络、方法关系和组合筛选。
...
- `paper.html` 集中展示单篇论文面向读者的完整方法、实验、结果、局限、复现、证据与引用信息；
- 组合筛选页支持对子问题进行多选，并按并集或交集筛选；
- 引用图从当前论文集合向引用前驱递归展开，执行任意深度的显示层传递约简，并支持总线分支、缩放、拖动和连线关系提示；
...
- `papers.html`：全部论文列表。
- `paper.html?id=medusa`：单篇论文完整条目与站内引用前驱。
...
引用图只从页面当前论文集合出发，沿 `citations` 递归加入被引用论文；不会反向加入仅仅在后来引用当前论文的工作。若一条引用边同时记录了 `extends`、`comparesAgainst`、`related` 或 `compatibleWith`，这些方法关系会作为连线标注显示；否则标为普通引用。

引用图只在浏览器显示层做传递约简：只要 `A → D` 已可由任意深度的 `A → B → … → D` 引用路径表达，就隐藏直接边。递归展开保证中间论文即使不属于当前筛选结果，也能参与这次判断。一个来源保留多条显示边时先经过同一段总线，再分成独立直角路径；处理不会改写任何源数据。
