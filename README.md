...
- `paper.html` 集中展示单篇论文的作者、直观方法概述、单位、详细贡献和引用图；
- 组合筛选页支持对子问题进行多选，并按并集或交集筛选；
- 引用图在显示时自动执行传递约简，按“右侧引用左侧”分层，并支持缩放与拖动；
...
引用图只在浏览器显示层做传递约简：若 `A → C` 已可由 `A → B → … → C` 表达，则隐藏直接的 `A → C` 连线。该处理不会改写 `citations`、生成目录或反向派生的 `citedBy`。每次筛选后都会针对当前结果集重新计算，因此中间论文不在结果集时，仍会保留必要的直接边。
