...
SDAtlas 是一个零依赖、可直接静态部署的投机解码论文图谱。站点以统一的 A–E“子问题”组织论文，同时提供完整论文目录、单篇论文页、站内论文关系和组合筛选。
...
- `paper.html` 集中展示单篇论文的作者、直观方法概述、单位、详细贡献和论文关系图；
- 组合筛选页支持对子问题进行多选，并按并集或交集筛选；
- 论文关系图按“右侧来源指向左侧目标”分层，对同类型关系执行显示层传递约简，并支持缩放、拖动和连线关系提示；
...
- `papers.html`：全部论文列表。
- `paper.html?id=medusa`：单篇论文详情与站内论文关系。
...
未知文本、布尔和枚举值使用 `null`；确认不存在的布尔值使用 `false`，集合使用空数组。本次迁移仅将尚未人工细分关系的旧条目按已有 `citations` 初始化到 `relations.related`；已人工整理的 `relations` 保持原样。该初始化是一次性基线，后续两组字段独立维护，未核实的实验、特征和方法关系不能根据相似论文猜测。
...
论文关系图从 `relations.extends`、`comparesAgainst`、`related` 和 `compatibleWith` 建边。同一对论文的多种关系合并为一条线；放大到详细视图会在线上显示字段名，悬停或键盘聚焦会显示完整关系提示。

关系图只在浏览器显示层、按 relation 类型做传递约简：只有同类型的 `A → C` 已可由 `A → B → … → C` 表达时，才隐藏直接边。纯 `related` 关系若被双方重复记录，则在显示时合并为一条稳定方向的边，避免产生没有层级意义的双向重复线。全部路径均使用直角折线和小型箭头；处理不会改写任何源数据。每次筛选后都会针对当前结果集重新计算，因此中间论文不在结果集时，仍会保留必要的直接边。
